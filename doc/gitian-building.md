...
*Setup instructions for a gitian build of Zubit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zubit
...
Clone the git repositories for zubit and gitian and then checkout the zubit version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zubit-crypto/zubit.git
cd zubit
...
Follow the instructions in [doc/release-process.md](release-process.md) in the zubit repository
...
To build Zubit (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the zubit repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zubit/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/zubit-crypto/zubit
...
For example:
```bash
URL=https://github.com/crowning-/zubit.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zubit=${COMMIT} --url zubit=${URL} ../zubit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zubit=${COMMIT} --url zubit=${URL} ../zubit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zubit=${COMMIT} --url zubit=${URL} ../zubit/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zubit-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zubit-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/zubit-build.assert
```
...
[zubit/gitian.sigs](https://github.com/zubit-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
