...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZBIT);
    ...
    if (!settings.contains("nAnonymizeZubitAmount"))
        settings.setValue("nAnonymizeZubitAmount", 1000);

    nAnonymizeZubitAmount = settings.value("nAnonymizeZubitAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZubitAmount"))
        SoftSetArg("-anonymizezubitamount", settings.value("nAnonymizeZubitAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zubit.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZubitAmount:
            return QVariant(nAnonymizeZubitAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZubitAmount:
            nAnonymizeZubitAmount = value.toInt();
            settings.setValue("nAnonymizeZubitAmount", nAnonymizeZubitAmount);
            emit anonymizeZubitAmountChanged(nAnonymizeZubitAmount);
            break;
        case CoinControlFeatures:
