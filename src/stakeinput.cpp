...
// The zZBIT block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zZBIT is a hash of the serial
...
{
    //Create an output returning the zZBIT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZZBITOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zZBIT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zZBIT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZZBITOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zZBIT output", __func__);
...
{
    CzZBITTracker* zzbitTracker = pwallet->zzbitTracker.get();
    CMintMeta meta;
    if (!zzbitTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zzbitTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a ZBIT stake is the outpoint
