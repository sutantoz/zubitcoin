...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zzbitTracker = unique_ptr<CzZBITTracker>(new CzZBITTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZZBIT_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZZBIT_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
