...
CzZBITTracker::CzZBITTracker(std::string strWalletFile)
{
...
CzZBITTracker::~CzZBITTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzZBITTracker::Init()
{
...
bool CzZBITTracker::Archive(CMintMeta& meta)
{
...
bool CzZBITTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzZBITTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzZBITTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzZBITTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzZBITTracker::GetSerialHashes()
{
...
CAmount CzZBITTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzZBITTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzZBITTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzZBITTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzZBITTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzZBITTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzZBITTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzZBITTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzZBITTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzZBITTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzZBITTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzZBITTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzZBITTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzZBITTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzZBITTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzZBITTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzZBITTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzzbit from DB\n", __func__, listDeterministicDB.size());
    }
