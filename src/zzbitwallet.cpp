...
CzZBITWallet::CzZBITWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zzbit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZZBITSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZZBITSeed_deprecated()) {
                    LogPrintf("%s: Updated zZBIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zzbit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zzbit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzZBITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZZBITCount(nCountLastUsed);
    else if (!walletdb.ReadZZBITCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzZBITWallet::Lock()
{
    seedMaster = 0;
}

void CzZBITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzZBITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZZBIT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzZBITWallet::LoadMintPoolFromDB()
{
...
void CzZBITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzZBITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzZBITWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zzbitTracker->HasPubcoinHash(pMint.first)) {
...
bool CzZBITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZZBIT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zzbitTracker which also adds to database
    pwalletMain->zzbitTracker->Add(dMint, true);
...
void CzZBITWallet::SeedToZZBIT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzZBITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzZBITWallet::GenerateDeterministicZZBIT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzZBITWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzZBITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
