...
#ifndef Zubit_ZZBITWALLET_H
#define Zubit_ZZBITWALLET_H
...
public:
    CzZBITWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZZBIT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZZBIT(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Zubit_ZZBITWALLET_H
